...
def main():
    ...
    seen_edges = {(e["source"], e["target"]) for e in t1_data["edges"]}
    print(f"Loaded {len(seen_edges):,} T1 edges to deduplicate against\n")
    ...
    # Pass 1: collect all published CVE IDs (lightweight, no data retained)
    all_cve_ids = set()
    for filepath in tqdm(json_files, desc="Collecting IDs"):
        try:
        ...
        if cve_id and meta.get("state") == "PUBLISHED":
            all_cve_ids.add(cve_id)

    print(f"\nFound {len(all_cve_ids):,} published CVE IDs")
    ...
    print(f"Combined total:               {len(t1_data['edges']) + len(new_edges):>8,}")
    print(f"{'='*50}")
